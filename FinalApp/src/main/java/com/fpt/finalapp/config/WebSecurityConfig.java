...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable();
		http.authorizeRequests().antMatchers("/", "/login","/themnv").permitAll();
		http.authorizeRequests().antMatchers("/trogiup", "/changepassword").access("hasAnyRole('r1', 'r2', 'r3')");
		http.authorizeRequests().antMatchers("/thongke").access("hasRole('r1', 'r2')");
//		http.authorizeRequests().antMatchers("/themnv", "/thongtinnhanvien", "/");
